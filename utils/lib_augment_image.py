...
class ImgAugmenter(object):
    ...
    def _put_object_onto_background(self, img_obj, mask_obj, img_bg):
        ...
        scale = args.rescale * \
            (get_len(img_bg) * args.rela_size) / get_len(img_obj)
        ...
        # format: BRG, same as input image
        new_obj = np.round(new_obj*255).astype(np.uint8)
