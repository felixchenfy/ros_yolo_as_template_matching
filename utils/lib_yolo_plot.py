...
class Yolo_Detection_Plotter_CV2(object):
    ...
    def plot(self, img, detections, img_channels='rgb', if_plot_center=True, if_print=True):
        '''
        Input:
            img {image}: 3 channels, rgb, uint8.
            detections {Nx7 arrays}: Info of each obj: Bbox(4), conf, cls_conf, cls_idx.
                This can be {2d list} or {np.ndarray} or {torch.Tensor}
            if_plot_center {bool}: Draw a circle at center.
        Output:
            self.img_disp {image}: 3 channels, bgr, uint8.
        '''

        # -- Check input.
        if img_channels in ["rgb", "RGB"]:
            # change color to BGR. This is what cv2.imshow needs.
            self.img_disp = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        else:
            self.img_disp = img.copy()

        # -- Plot.
        self._plot_onto_image(self.img_disp, detections,
                              if_plot_center, if_print)
    ...
    def _plot_onto_image(self, img, detections, if_plot_center, if_print):
        ...
        if (detections is not None) and (len(detections) > 0):
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_idx in detections:
                ...
                # Draw rectangular
                x1, y1, width, height = map(
                    int, [x1, y1, x2-x1, y2-y1])  # to int
                ...
                if if_print:
                    print("\t  box: center=({:.1f}, {:.1f}), w={:.1f}, h={:.1f}".format(
                        xc, yc, width, height))
